#defa ispiraky vo recherche vo extension des idées anie zany gn sisa @ design frontend zao; magnatsara an'azy sisa fa defa mianatry gn backend zany gn tokony ataoko


'''
c'est fini, il nous reste que de l'amelioration pour le frontend; alors où est ce qu'on en est jusqu'ici par rapport la structure; je pense qu'on peut dire fini pour coté backend, n'est ce pas, il nous reste que le frontend flask et api rest flask + api react; en fait je ne sais pas trop mais je sais juste que leur relation et sans oublier que la base de données est mysql et aussi surtout le plus important est qu'on soit en microservice avec 4 base de données pour les 4 services; voici rappel de structure de tout à l'heure:
...
#j'ai laissé tombé le Sidebar
#mbola tsisy idiran'i conception hatreto ah kkkk
#tsy de arakiko io AuthContext io fa ça va arakiko avao
...
│   ├── Footer.js              # Terminé
│   ├── CardFormations.js      # Terminé 
│   ├── Sidebar.js             # A faire => //navelako fa mitony @ navbar ihany
├── pages/
│   ├── Home.js                # À faire =>//azokazoko fotsiny fa mila recherche sy personnalisation, tsy vita izy fa misy fotsiny
│   ├── Login.js               # Terminé
│   ├── Register.js            # Terminé
│   ├── Formations.js          # Terminé
│   ├── Cart.js                # Terminé
│   ├── Checkout.js            # À faire => misy fotsiny fa mbola tsy vita, à personnaliser zany eh
│   ├── Admin.js               # À faire => efa misy koa fa à personnaliser
├── services/
│   ├── userService.js         # Basique (à connecter)
│   ├── formationService.js    # Terminé (mocks)
│   ├── paymentService.js      # À faire => simulation
│   ├── orderService.js        # À faire => simulation  mbola tsisy ihany reto service reto an, juste le formation rery o sur hatreto
├── context/
│   ├── AuthContext.js         # À faire => miankina @ backend fa efa misy simulation
│   ├── CartContext.js         # Terminé mais juste pour mocks
├── assets/
│   ├── images/                # En cours
│   ├── styles/                # En cours
...
'''
et où est ce qu'on en est avec les 3 service dans ce frontend, formation c'est terminé mais pour payment,order,userservice commenrt? et c'est quoi exactement de order? ou est ce qu'il sont deja fini ao nom de cart,ckeckout login et register? il ne reste juste que parie backend? je veux que tout soit bien clair pour le frontend, toujour en basant sur la structure frontend
'''
...
'''
#zayyy, tsisy miverimberina koa
#mbola hasiako icone olona na personne aby @ navbar io @zay mba tsara2 kokoa izy @zay