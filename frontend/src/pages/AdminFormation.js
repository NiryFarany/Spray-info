...
//ok probleme ecriture ajout et modifié formation, merci claude ai kkkkk
// ✅ COMPOSANT DÉPLACÉ EN DEHORS (évite les re-renders inutiles)
...
const AdminFormation = () => {
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      ...
      setMessage("Tous les Formations");
          ...
          {formations.map((formation) => (
            <tr key={formation.id} className="border-b hover:bg-gray-50">
              ...
              <td className="py-3 px-6 flex justify-center gap-3">
                <EyeIcon 
                  className="h-6 w-6 cursor-pointer text-blue-600 hover:scale-110"
                  onClick={() => handleView(formation)} 
                />
                <PencilIcon 
                  className="h-6 w-6 cursor-pointer text-green-600 hover:scale-110"
                  onClick={() => handleEdit(formation)} 
                />
                <TrashIcon 
                  className="h-6 w-6 cursor-pointer text-red-600 hover:scale-110"
